-- Active: 1774204402905@@127.0.0.1@5432@test_db
...
CREATE TABLE merchants (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT merchants_email_tenant_unique UNIQUE (email, tenant_id)
);
...
CREATE TABLE cashiers (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT cashiers_cashier_id_tenant_unique UNIQUE (cashier_id, tenant_id),
    CONSTRAINT cashiers_cashier_id_not_empty CHECK (btrim(cashier_id) <> '')
);

CREATE INDEX idx_cashiers_tenant_id ON cashiers (tenant_id);
...
CREATE TABLE customers (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT customers_phone_not_empty CHECK (btrim(phone) <> '')
);
...
CREATE TABLE orders (
    ...
    cashier_id UUID REFERENCES cashiers (id) ON DELETE RESTRICT,
...
CREATE TABLE otp_tokens (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT otp_tokens_phone_not_empty CHECK (btrim(phone) <> ''),
    CONSTRAINT otp_tokens_otp_format CHECK (otp ~ '^[0-9]{6}$')
);

CREATE INDEX idx_otp_tokens_lookup ON otp_tokens (
    phone,
    otp,
    used,
    expires_at DESC,
...
INSERT INTO
    admins (email, name, password_hash)
VALUES (
        'jack@dev.local',
        'Platform Admin',
